...
ssh -o StrictHostKeyChecking=no -i deployer-ssh-key/deployer deployer@${TARGETIP} "rsync -e 'ssh -o StrictHostKeyChecking=no -i .ssh/deployer' --rsync-path='sudo rsync' --chown=nobody:nobody --chmod=D2775,F664 -rlptgD --recursive --progress --numeric-ids --delete --omit-dir-times --exclude=wp-config.php deployer@${SOURCEIP}:${SITEDIR} /var/www/"
