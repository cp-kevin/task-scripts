...
grep DB_NAME wp-config.php | cut -d \' -f4 > www-old-info/old_db_name
export OLDDBNAME=$(cat www-old-info/old_db_name)

# compress would be improvement
ssh -o StrictHostKeyChecking=no -i deployer-ssh-key/deployer deployer@${SOURCEIP} "mysqldump --defaults-extra-file=/home/deployer/mysql_creds.cnf -h 127.0.0.1 -P 6033 --single-transaction --no-create-db ${OLDDBNAME} > ${SITEDIR}/tmp_db_dump.sql; rm /home/deployer/mysql_creds.cnf"
