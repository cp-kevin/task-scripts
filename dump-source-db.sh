...
ssh -o StrictHostKeyChecking=no -i deployer-ssh-key/deployer deployer@${SOURCEIP} "mysqldump --defaults-extra-file=/home/deployer/mysql_creds.cnf -h 127.0.0.1 -P 6033 --single-transaction --no-create-db ${DBNAME} > ${SITEDIR}/tmp_db_dump.sql; rm /home/deployer/mysql_creds.cnf"
